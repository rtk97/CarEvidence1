...
public class Reflection {
    ...
    public Reflection() {
        ...
        this.entityDefinition = typePool.describe("pl.edu.wat.carevidence.entity.Wypozyczajacy").resolve();
        this.requestDefinition = typePool.describe("pl.edu.wat.carevidence.dto.WypozyczajacyRequest").resolve();
        this.responseDefinition = typePool.describe("pl.edu.wat.carevidence.dto.WypozyczajacyResponse").resolve();
        this.mapperDefinition = typePool.describe("pl.edu.wat.carevidence.mapper.WypozyczajacyMapper").resolve();
    ...
    public static void apply(String fieldName, String fieldType) {
        ...
        ref.applyWypozyczajacyMapper(fieldName);
    }

    private void applyWypozyczajacyMapper(String fieldName) {
        TypePool typePool = TypePool.Default.ofSystemLoader();
        ByteBuddy byteBuddy = new ByteBuddy();
        DynamicType.Builder<Object> builder = byteBuddy
                .redefine(mapperDefinition,
                        ClassFileLocator.ForClassLoader.ofSystemLoader())
                .method(named("fillWypozyczajacyRequest"))
                .intercept(MethodCall.invoke(setterWypozyczajacyEntity(fieldName))
                        .onArgument(0)
                        .withMethodCall(MethodCall
                                .invoke(getterRequest(fieldName))
                                .onArgument(1)))
                .method(named("fillWypozyczajacy"))
                .intercept(MethodCall.invoke(setterWypozyczajacyResponse(fieldName))
                        .onArgument(0)
                        .withMethodCall(MethodCall
    ...
    private MethodDescription setterWypozyczajacyResponse(String fieldName) {
    ...
    private MethodDescription setterWypozyczajacyEntity(String fieldName) {
        return entityDefinition
                .getDeclaredMethods()
                .filter(ElementMatchers.isSetter(fieldName))
                .stream()
    ...
    private void applyResponse(String fieldName, String fieldType) {
        ...
        try (var unloadedWypozyczajacy = builder.make()) {
            responseDefinition = unloadedWypozyczajacy.load(ClassLoader.getSystemClassLoader(), ClassLoadingStrategy.Default.INJECTION)
                    .getTypeDescription();
    ...
    public void applyEntity(String fieldName, String fieldType) {
        ...
        try (var unloadedWypozyczajacy = builder.make()) {
            entityDefinition = unloadedWypozyczajacy.load(ClassLoader.getSystemClassLoader(), ClassLoadingStrategy.Default.INJECTION)
                    .getTypeDescription();
